...
/**
 * Handles image upload to Supabase storage with progress tracking and abort capability
...
export const handleImageUpload = async (
  ...
  try {
    ...
    // Get signed URL for the uploaded image (valid for 1 year)
    const { data: signedUrlData, error: signedUrlError } = await supabase.storage
      .from('images')
      .createSignedUrl(data.path, 31536000) // 1 year in seconds

    if (signedUrlError) {
      throw new Error(`Failed to get image URL: ${signedUrlError.message}`)
    }
...
/**
 * Retrieves a signed URL for an image stored in Supabase
